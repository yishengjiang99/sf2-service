...
export async function load(url){
	const Module=await import("./build/pdta.js");
	const module=await Module.default();
	const {pdtaBuffer, nsamples,sdtaStart, infos} =await sfbkstream(url);
	...
	function getShdr(samleId) {
		...
		const range='bytes='+(sdtaStart+start*2)+"-"+(sdtaStart+end*2+1);
		const loops= [ (startloop-start)/sampleRate,(endloop-start)/sampleRate];
...
//		return [start, end, startloop, endloop, sampleRate, originalPitch, pitchCorrection];
	...
	function *zoneSampleHeaders(zref){
		let lastSampleId;
		let zones=[];
		for(let zoneProxy = zref2Zone(zref); zoneProxy.SampleId >=0; zoneProxy=zref2Zone(zref+=120)){
	...
	function setProgram(pid, channelId) {
		...
		chmap[channelId] =  module.ccall("findByPid", "number", ["u16", "u16"], [pid, bkid], null);
